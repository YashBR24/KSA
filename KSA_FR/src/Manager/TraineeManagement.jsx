...
const TraineeManagement = () => {
  ...
  useEffect(() => {
    ...
    fetchSports();
    fetchInstitutes();
    fetchBatches(); // Fetch batches on mount
  }, []);
  ...
  const getBatchName = (batchId) => {
    ...
    return batch ? `${batch.name} (${batch.start_time} - ${batch.end_time})` : batchId || "N/A";
  };
  ...
  const handleRenewalSubmit = async (e) => {
    ...
    try {
      ...
      if (!renewalFormData.institute_id || !renewalFormData.sport_id || !renewalFormData.batch_id) {
        setError("Please select an institute, sport, and batch.");
        return;
      }
    ...
    setTraineeFormData(
      trainee
        ? {
            ...
            sport_id: trainee.sport_id || "",
            institute_id: trainee.institute_id || "",
            batch_id: trainee.batch_id || "", // Added batch_id
            plan_id: trainee.plan_id || "",
            photo: null,
            traineeSignature: null,
            fatherSignature: null,
  ...
  const exportToExcel = () => {
    ...
    const excelData = trainees.map((trainee) => ({
      ...
      Sport: getSportName(trainee.sport_id),
      Institute: getInstituteName(trainee.institute_id),
      Batch: getBatchName(trainee.batch_id), // Added batch
      "Start Date": moment(trainee.start_date).format("DD-MM-YYYY"),
      "Expiry Date": moment(trainee.expiry_date).format("DD-MM-YYYY"),
      "Amount Paid": `₹${trainee.amount}`,
      Active: trainee.active ? "Yes" : "No",
          ...
          >
            <div className="flex items-start p-4 gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-600">
                  Batch: <span className="font-medium">{getBatchName(trainee.batch_id)}</span>
                </p>
                <p className="text-sm text-gray-600">
                  Expires: <span className="font-medium">{formatDateTime1(trainee.to)}</span>
                </p>
          ...
          <div className="bg-white rounded-xl shadow-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <DetailItem label="Sport" value={getSportName(selectedTrainee.sport_id)} />
                <DetailItem label="Institute" value={getInstituteName(selectedTrainee.institute_id)} />
                <DetailItem label="Batch" value={getBatchName(selectedTrainee.batch_id)} /> {/* Added batch */}
              </div>
            </div>
          </div>
        </div>
