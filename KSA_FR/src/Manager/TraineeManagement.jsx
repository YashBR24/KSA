...
const TraineeManagement = () => {
  ...
  const fetchTrainees = async () => {
    try {
      ...
      // Sort trainees by roll number numerically
      normalizedTrainees.sort((a, b) => {
        const numA = parseInt(a.roll_no.replace("KSA", ""), 10);
        const numB = parseInt(b.roll_no.replace("KSA", ""), 10);
        return numA - numB;
      });
      setTrainees(normalizedTrainees);
      console.log("Fetched Trainees:", normalizedTrainees);
    } catch (error) {
      console.error("Error fetching Students:", error);
      setError(
        error.response?.data?.message ||
  ...
  const getBatchName = (batchId) => {
    const batch = batches.find((b) => b._id === batchId);
    return batch
      ? `${batch.name} (${formatTime(batch.start_time)} - ${formatTime(
          batch.end_time
        )})`
      : batchId || "N/A";
  };
  ...
  const handleTraineeInputChange = (e) => {
    ...
    if (name === "sport_id") {
      ...
      updatedFormData.amount = selectedPlan ? selectedPlan.amount : 0;
      updatedFormData.initial_payment =
        updatedFormData.payment_status === "PAID"
  ...
  const handleRenewalInputChange = (e) => {
    ...
    if (name === "sport_id") {
      ...
      updatedFormData.initial_payment =
        updatedFormData.payment_status === "PAID"
          ? updatedFormData.amount
          : updatedFormData.payment_status === "PENDING"
          ? 0
          : updatedFormData.initial_payment;
      updatedFormData.expiry_date = expiryDate;
    } else if (name === "start_date") {
  ...
  const handleTraineeSubmit = async (e) => {
    ...
    if (
      traineeFormData.payment_status === "PARTIAL" &&
      (!traineeFormData.initial_payment ||
        traineeFormData.initial_payment <= 0 ||
        traineeFormData.initial_payment >= traineeFormData.amount)
    ) {
      setError(
        "Initial payment must be greater than 0 and less than total amount for PARTIAL status."
      );
      setIsSubmitting(false);
      return;
    }
  ...
  const handleRenewalSubmit = async (e) => {
    ...
    if (
      renewalFormData.payment_status === "PARTIAL" &&
      (!renewalFormData.initial_payment ||
        renewalFormData.initial_payment <= 0 ||
        renewalFormData.initial_payment >= renewalFormData.amount)
    ) {
      setError(
        "Initial payment must be greater than 0 and less than total amount for PARTIAL status."
      );
          ...
          >
            <div className="flex items-start p-4 gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-600">
                  Expires:{" "}
                  <span className="font-medium">{formatDate(trainee.to)}</span>
                </p>
                <p className="text-sm text-gray-600">
                  Payment Status:{" "}
                  <span
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 overflow-y-auto">
          <div className="min-h-screen px-4 py-8 flex items-center justify-center">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md relative">
              ...
              <div className="p bbox-shadow-6 sm:p-8">
                <h3 className="text-2xl font-semibold mb-8 text-gray-800 text-center">
                  Add Partial Payment
                </h3>
                <form onSubmit={handlePaymentSubmit} className="space-y-6">
