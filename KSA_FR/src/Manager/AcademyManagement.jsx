...
const AcademyManagement = () => {
  ...
  const [showBatchPopup, setShowBatchPopup] = useState(false); // New popup for batches
  const [editingPlan, setEditingPlan] = useState(null);
  const [editingSport, setEditingSport] = useState(null);
  const [editingInstitute, setEditingInstitute] = useState(null);
  const [editingBatch, setEditingBatch] = useState(null); // New state for editing batch
  const [showPlans, setShowPlans] = useState(false);
  const [showSports, setShowSports] = useState(false);
  const [showInstitutes, setShowInstitutes] = useState(false);
  const [showBatches, setShowBatches] = useState(false); // New state for batch section
  const [showTrainee, setShowTrainee] = useState(true);
  const [formData, setFormData] = useState({
    name: "",
    amount: "",
  ...
  const [batchFormData, setBatchFormData] = useState({
    ...
    end_time: "",
    active: true,
  }); // New state for batch form

  const toggleSection = (section) => {
    if (section === "plans") setShowPlans(!showPlans);
    else if (section === "sports") setShowSports(!showSports);
    else if (section === "institutes") setShowInstitutes(!showInstitutes);
    else if (section === "batches") setShowBatches(!showBatches); // Toggle batch section
    else if (section === "trainee") setShowTrainee(!showTrainee);
  };
  ...
  const handleSubmitBatch = async (e) => {
    ...
    try {
      if (editingBatch) {
        ...
        await axios.post(`${ip}/api/academy/add-batch`, batchFormData);
      }
      setBatchFormData({ name: "", start_time: "", end_time: "", active: true });
