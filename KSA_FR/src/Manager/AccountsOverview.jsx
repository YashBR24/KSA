...
//         `${ip}/api/accounts/transaction/calculate-balance`,
//         {
//           userId: localStorage.getItem("userid"),
//           instituteId,
//           startDate: filters.fromDate,
...
//   const calculateMethodAndDateTotals = (data) => {
//     // Calculate method-wise totals
//     const methodTotals = {
//       CASH: 0,
//       UPI: 0,
//       CARD: 0,
...
//       // Method-wise totals
//       if (account.method && methodTotals.hasOwnProperty(account.method)) {
//         methodTotals[account.method] += Number(account.amount || 0);
//       }

//       // Date-wise totals
//       const date = new Date(account.createdAt).toLocaleDateString("en-GB", {
//         day: "2-digit",
//         month: "2-digit",
//         year: "numeric",
//       });
...
//       }
//       dateWiseTotals[date] += Number(account.amount || 0);
//     });

//     setMethodTotals(methodTotals);
//     setDateWiseTotals(dateWiseTotals);
//   };
...
//     const filteredWithBalances = applyFiltersAndCalculateBalances(accounts);
//     setFilteredData(filteredWithBalances);
//     calculateMethodAndDateTotals(filteredWithBalances);
//     setCurrentPage(1);
//   }, [accounts, filters]);
...
//     autoTable(doc, {
//       head: [
//         ["#", "Date", "Type", "Amount", "Description", "Method"],
//       ],
//       body: filteredData.map((account, index) => [
//         index + 1,
//         new Date(account.createdAt).toLocaleString(),
...
//                     {Object.keys(dateWiseTotals).length === 0 ? (
...
const AccountsOverview = () => {
  ...
  const [sportTotals, setSportTotals] = useState({});
  ...
  const calculateMethodAndSportTotals = (data) => {
    // Calculate method-wise totals
    const methodTotals = {
      CASH: 0,
      UPI: 0,
    ...
    data.forEach((account) => {
      ...
      if (account.description) {
        ...
        sports.forEach((sport) => {
          if (sport.name && descriptionLower.includes(sport.name.toLowerCase())) {
            if (!sportTotals[sport.name]) {
              sportTotals[sport.name] = 0;
            }
            sportTotals[sport.name] += Number(account.amount || 0);
          }
        });
      }
    });
  ...
  useEffect(() => {
    const filteredWithBalances = applyFiltersAndCalculateBalances(accounts);
    setFilteredData(filteredWithBalances);
    calculateMethodAndSportTotals(filteredWithBalances);
    setCurrentPage(1);
  }, [accounts, filters, sports]);
  ...
  return (
    <div className="min-h-screen bg-transparent p-0 md:p-6">
      <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto">
                    {Object.keys(sportTotals).length === 0 ? (
                      <p className="text-sm text-gray-500">
                        No sport-related transactions available
                      </p>
                    ) : (
        ...
        {showExpenseForm && (
          <div className="fixed inset-0 bg-black bg-opacity-10 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow folders-lg max-w-md w-full p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold">Add Transaction</h2>
                <button
                  onClick={closeExpenseForm}
