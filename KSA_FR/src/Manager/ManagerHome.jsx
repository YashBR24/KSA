...
const StatCard = ({ icon: Icon, title, value, trend, trendDirection, comparisonText, onClick }) => (
    <div
        onClick={onClick}
        className="group bg-white rounded-2xl p-6 shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100 cursor-pointer"
    >
        <div className="flex justify-between items-start">
            <div className="space-y-2">
                <p className="text-gray-600 text-sm font-medium">{title}</p>
            ...
            <div
                className={`p-3 rounded-xl transform group-hover:scale-110 transition-transform ${
                    trendDirection === "up" ? "bg-green-100" : "bg-red-100"
                }`}
            >
                <Icon
                    className={`w-6 h-6 ${trendDirection === "up" ? "text-green-600" : "text-red-600"}`}
                />
            </div>
        </div>
        {trend !== undefined && (
...
const QueryCard = ({ query }) => {
    ...
    return (
        <div className="group bg-white rounded-2xl p-4 shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100">
            <h3 className="text-lg font-semibold text-gray-800">{query.name}</h3>
            <p className="text-sm text-gray-600">Email: {query.email}</p>
            <p className="text-sm text-gray-600">Mobile No: {query.mobile_no}</p>
            <p className="text-sm text-gray-600">Description: {query.description}</p>
...
const EventCard = ({ event }) => {
    ...
    return (
        <div className="group bg-white rounded-2xl p-4 border border-gray-100 hover:shadow-lg transition-all duration-300">
            <div className="flex items-start justify-between">
                <div>
                    <h3 className="font-semibold text-gray-900">{event.name}</h3>
                    <div className="mt-2 space-y-1">
                ...
                <div className="bg-blue-50 p-2 rounded-lg transform group-hover:scale-110 transition-transform">
                    <Trophy className="w-6 h-6 text-blue-600" />
                </div>
            </div>
        </div>
            ...
            <div className="space-y-4">
                <div className="relative">
                    <input
                        ...
                        onChange={(e) => setRollno(e.target.value)}
                        placeholder="Enter Roll Number"
                        className="w-full px-4 py-2 rounded-lg border border-gray-300 bg-white text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
                        onKeyPress={(e) => e.key === "Enter" && markAttendance()}
                        aria-label="Enter student roll number for attendance"
                    />
                    <button
                        onClick={markAttendance}
                        className="absolute right-2 top-1/2 -translate-y-1/2 bg-gradient-to-r from-blue-500 to-blue-600 text-white p-2 rounded-lg hover:from-blue-600 hover:to-blue-700 transition-all"
            ...
            <div className="space-y-4">
                <div className="relative">
                    <input
                        ...
                        placeholder="Enter 8-digit Roll Number"
                        className="w-full px-4 py-2 rounded-lg border border-gray-300 bg-white text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
                        aria-label="Search by 8-digit roll number"
                    />
                    <Search className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" />
                </div>
...
const RevenueChart = ({ data }) => {
    ...
    return (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div className="flex justify-end mb-4">
                <select
                    className="border border-gray-300 bg-white text-gray-900 p-2 rounded-lg focus:ring-2 focus:ring-blue-500"
                    value={chartType}
    ...
    return (
        <div className="group bg-white rounded-2xl p-4 border border-gray-100 hover:shadow-lg transition-all duration-300">
            <div className="flex justify-between items-start">
                ...
                <div
                    className={`p-2 rounded-lg transform group-hover:scale-110 transition-transform ${
                        booking.payment_status === "Pending"
                            ? "bg-yellow-50 text-yellow-600"
                            : "bg-green-50 text-green-600"
                    }`}
...
// ExpiringAdmissionCard Component
const ExpiringAdmissionCard = ({ admission }) => {
    ...
    // Find the sport name by matching admission.sportId with sports.id
    const sportName = sports.find((sport) => sport.id === admission.sportId)?.name || "Unknown Sport";
            ...
            <div className="flex justify-between items-start">
                <div className="space-y-2">
                    ...
                    <div className="space-y-1">
                        ...
                        <p className="text-sm text-gray-600">
                            <span className="font-medium">Sport:</span> {sportName}
                        </p>
                        <p className="text-sm text-gray-600">
                ...
                <div className="p-2 rounded-lg bg-purple-50 text-purple-600 transform group-hover:scale-110 transition-transform">
                    <UserCheck className="w-6 h-6" />
                </div>
            </div>
        </div>
...
// ExpiringAdmissionsSection Component
const ExpiringAdmissionsSection = ({ admissions, onNavigate }) => {
    ...
    const uniqueSessions = [...new Set(admissions.map((admission) => admission.session))];
    ...
    useEffect(() => {
        ...
        if (filterDays) {
            filtered = filtered.filter((admission) => {
                const daysLeft = Math.ceil((new Date(admission.to) - new Date()) / (1000 * 60 * 60 * 24));
                return filterDays === "3"
                    ? daysLeft <= 3
                    : filterDays === "7"
                    ? daysLeft <= 7
        ...
        if (searchQuery) {
            filtered = filtered.filter(
                (admission) =>
                    ...
                    admission.roll_no.toLowerCase().includes(searchQuery.toLowerCase())
            );
        }

        // Filter by session
        if (filterSession) {
            filtered = filtered.filter((admission) => admission.session === filterSession);
        }

        setFilteredAdmissions(filtered);
    }, [admissions, filterDays, searchQuery, filterSession]);
    ...
    const exportToExcel = () => {
        const data = filteredAdmissions.map((admission) => ({
            Name: admission.name,
            "Roll No": admission.roll_no,
            Session: admission.session,
            Phone: admission.phone,
            School: admission.name_of_school,
            "Expiry Date": new Date(admission.to).toLocaleDateString("en-US", {
                day: "numeric",
                month: "short",
                year: "numeric",
            }),
            "Days Left": Math.ceil((new Date(admission.to) - new Date()) / (1000 * 60 * 60 * 24)),
        }));
        ...
        XLSX.writeFile(workbook, `Expiring_Admissions_${new Date().toISOString().split("T")[0]}.xlsx`);
    };
        ...
        <div  className="cursor-pointer">
            <div className="flex justify-between items-center mb-4">
                ...
                <button
                    ...
                    aria-label="Export expiring admissions to Excel"
                >
            ...
            {/* Filter Bar */}
            <div className="bg-white rounded-2xl p-4 mb-4 shadow-sm border border-gray-100 sticky top-0 z-10">
                <div className="flex flex-col md:flex-row gap-4 items-center">
                    <div className="relative flex-1">
                        <input
                            ...
                            className="w-full px-4 py-2 rounded-lg border border-gray-300 bg-white text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
                            title="Search by student name or roll number"
                    ...
                    {/* <select
                        ...
                        className="px-4 py-2 rounded-lg border border-gray-300 bg-white text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
                        title="Filter by days left until expiry"
                        aria-label="Filter by days left"
                    >
                        <option value="">All Days</option>
                        <option value="1">Expiring in 1 Days</option>
            ...
            {filteredAdmissions.length > 0 ? (
                ...
                <div className="bg-white rounded-2xl p-4 border border-gray-100 text-center text-gray-600">
...
const Dashboard = ({ onNavigate }) => {
    ...
    const fetchDashboardData = async () => {
        try {
            const response = await axios.post(`${ip}/api/admin/home/home`, {
                userid: localStorage.getItem("userid"),
            });
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-6 font-sans">
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                >
                    <StaffInfoCard totalStaff={dashboardData.totalStaff} totalCoaches={dashboardData.totalCoaches} />
                </div>
                ...
                {/* Expiring Admissions */}
                {dashboardData.ExpiringAdmissions && dashboardData.ExpiringAdmissions.length > 0 && (
                    <ExpiringAdmissionsSection admissions={dashboardData.ExpiringAdmissions} onNavigate={handleNavigation} />
                )}
            </div>
        </div>
    );
